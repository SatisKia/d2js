...
printf(",");
				cur2 = line;
				if ( find_line(&cur2, "dif(") ) {	// ägéUåı 0Å`1
					word(&cur2, tmp);
					printf("%s", f(tmp, scale));
				}
printf(",");
				cur2 = line;
				if ( find_line(&cur2, "amb(") ) {	// é¸àÕåı 0Å`1
					word(&cur2, tmp);
					printf("%s", f(tmp, scale));
				}
printf(",");
				cur2 = line;
				if ( find_line(&cur2, "emi(") ) {	// é©å»è∆ñæ 0Å`1
					word(&cur2, tmp);
					printf("%s", f(tmp, scale));
				}
printf(",");
				cur2 = line;
				if ( find_line(&cur2, "spc(") ) {	// îΩéÀåı 0Å`1
					word(&cur2, tmp);
					printf("%s", f(tmp, scale));
				}
printf(",");
				cur2 = line;
				if ( find_line(&cur2, "power(") ) {	// îΩéÀåıÇÃã≠Ç≥ 0Å`100
					word(&cur2, tmp);
					printf("%s", f(tmp, scale));
				}
